...
import './gcc_table_vm.dart';

// ignore: must_be_immutable
class GCCTableVU<T> extends StackedView<GCCTableVM> {
  GCCTableVU(
    this.actions, {
    Key? key,
  ...
  Future<dynamic> Function(Map<String, dynamic> v) loadData;
  Widget Function(T p) cellDesign;
  // loadData loadData(Map<String, dynamic> v) {
  ...
  @override
  Widget builder(BuildContext context, GCCTableVM viewModel, Widget? child) {
    return CHIScaffold(
      body: Column(
        children: [
          Visibility(
  ...
  @override
  GCCTableVM viewModelBuilder(BuildContext context) {
    GCCTableVM viewModel = GCCTableVM<T>(pageSize, apiController, refreshKey);
    loadDataOfVU(viewModel); //load for the first time.
    return viewModel;
  }

  void onRefresh(GCCTableVM viewModel) async {
  ...
  loadDataOfVU(GCCTableVM viewModel) async {
