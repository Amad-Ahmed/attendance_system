...
class LinearMeter extends Meter {
  ...
  LinearMeter(List<double> limits, List<Color> rangeColors, String label,
      ...
      : super(limits, rangeColors, label, value, unit, labelFontSize,
            unitFontSize);
  ...
  void textPainter(Canvas canvas, Size size) {
    textPaint(canvas, size, labelFontSize, label, size.width * labelPlacement,
        size.height * labelPlacement, true);
    String vu = value == 0 ? '--' : ('$value $unit');
    textPaint(canvas, size, unitFontSize, vu, size.width * labelPlacement,
        size.height * valuePlacement, false);
  }
}
